...
`taxadb` abbreviation | name | update frequency | synthetic
...
`iucn` | [The IUCN Red List](https://www.iucnredlist.org) of endangered species status
...
- `taxonRank`: the rank (as given by the provider) of this taxon. **Please note**: While DarwinCore specifies seven ranks as seperate columns (see below), many providers recognize many more of possible `taxonRank` values, such as "superclass", "superorder."  For example, NCBI (`ncbi`) and OpenTree Taxonomy (`ott`) recognize over 40 different ranks, many of which are unnamed, while Catalogue of Life (`col`), GBIF an others recognize only the seven principle ranks. Vastly controdictory claims between naming providers about what rank a given name belongs to or what species are included in which rank are common.  
