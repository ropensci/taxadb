...
## Render output without ANSI colour. Otherwise tibble and cli emit escape
## sequences that GitHub does not interpret, so they appear in README.md as
## literal "[90m" noise around every printed column.
...
If you use `taxadb` in published work, please cite the paper describing it,
and the naming provider whose data you used (`taxadb_provider_info()` gives
each provider's preferred citation):
...
if (requireNamespace("codemetar", quietly = TRUE)) codemetar::write_codemeta()
```
