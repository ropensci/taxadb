...
It is not necessary to call `td_create()`.  Specify `db=NULL` on any `taxadb` function to force this in-memory dispatch. 
...
KN: I don't know if this section needs to be in here, especially since `synonym()` doesn't fit cleanly in the four families of functions described at the beginning (maybe it's a helper function?). Can instead use the unresolved names discussion to segue into `by_` functions like in the tutorial. Something along the lines of "A name unresolved with `get_ids` could be due to misspelling, a missing synonym in the provider database OR a name resolving to multiple synonyms - 'by_' gives you access to all the synonym options." When it's laid out like that it also makes 'synonym()' seem less necessary.
