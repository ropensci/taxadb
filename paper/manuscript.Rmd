...
```{r}
unmatched$input %>% synonyms("iucn")
```


```{r}
unmatched$input %>% ids("iucn") %>% 
  select(scientificName, taxonomicStatus, input, taxonID, acceptedNameUsageID)
...
syn <- taxa_tbl("iucn") %>% 
  right_join(
    ...
    copy = TRUE) %>%
  select(scientificName, acceptedNameUsageID, taxonomicStatus) %>%
  collect()



## Some names have multiple synonyms, do not do this as a spread...    
syn %>% mutate(n = 1:length(scientificName)) %>% spread(taxonomicStatus, scientificName)
```
