...
and a growing number of databases and tools have emerged and grown over the past few decades
...
automatically downloaded and imported and stored on a local database by `taxadb`.  
...
After loading the package, we look up the taxonomic identifier for a Atlantic Cod, *Gadus morhua*, and the compliment:
...
joined %>%
  printtable(caption = "IUCN and trait data joined on scientific name.")
```
