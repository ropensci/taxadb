...
`taxadb` can also resolve common names to their identifier by mapping common name to the accepted scientific name. Common names have many of the same issues as scientific names even more frequently (e.g. matching to more than one accepted name, non-standardized formatting). Common names are accessed via `filter_common` which takes a vector of common names. The user can then resolve discrepancies. 
