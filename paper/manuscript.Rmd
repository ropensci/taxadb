...
```{r}
unmatched %>% mutate(iucn = get_ids(input, "iucn", "prefix"))
```
...
We can look for these synonyms in other tables too. ITIS recognizes some of these unmatched names as synonyms and some as accepted names. 

```{r}
unmatched$input %>% synonyms("itis")

unmatched$input %>% ids("itis")

## 
syn$synonym %>% na.omit() %>% unique() %>% synonyms("itis") 
