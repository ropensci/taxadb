...
tibble(provider = c("Integrated Taxonomic Information System", 
                    ...
                    total_identifiers = map(abbreviation, 
                                            ~taxa_tbl(.x) %>% 
                                              select(acceptedNameUsageID) %>% pull() %>% n_distinct() %>% unlist())
) %>% printtable()
