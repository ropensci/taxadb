...
These challenges have become particularly acute as it becomes increasingly common for researchers to work across a larger number and diversity of species in any given analysis, which may preclude the resources or substantative taxonomic expertise for all clades needed to resolve scientific names [@Patterson2010].  

While these issues have long been recognized in the literature [@boyle2013; @dayrat2005; @bortolus2008; @maldonado2015], and a growing number of databases and tools have emerged and grown over the past few decades [e.g. @itis; @ncbi; @col; @rees2014; @alvarez2018; @wagner2016; @foster2018], it remains difficult to resolve taxonomic names to a common authority in a transparent, efficient, and automatable manner.
...
Users can also explicitly trigger this one-time setup using `td_create()` and specifying the provider abbreviation (see Table 6), or simply using `all` to install all available providers:
...
status_data %>% printtable(caption = "IUCN status for the selected 10 species.")
```

```{r trait_table, echo = FALSE, cache = FALSE }
trait_data %>% printtable(caption = "Trait data for the selected 10 species.")
```


If we attempted to join these data directly on the species names provided by each table, we would find very little overlap, with only one species having both a body mass and an IUCN threat status resolved (Table 3). 
...
joined %>% tidyr::replace_na(list(category = "-")) %>% 
  printtable(caption = "IUCN and trait data joined on scientific name.") # "NA" looks like a category, "-" is clearer visually
```
...
As well as giving a detailed look at specific species or ids. For example, why can't `get_ids` resolve a seemingly common species?

```{r}
multi_match <- filter_name("Zosterops rendovae")
```

```{r echo = FALSE, cache = FALSE}
multi_match %>%
  select(1:5) %>%
  printtable(caption = "Accepted name matches for Zosterops rendovae.")
```

We see that *Zosterops rendovae* matches two accepted names which the user will have to choose between (Table 5).
...
desc <- c(itis = "originally formed to standardize taxonomic name usage across many agencies in the United States federal government",
          ...
          col = "comphrehensive taxonomic effort, includes some other providers (e.g. itis)",
          ...
          ott = "comphrenesive tree of life based on phylogenetic trees and taxonomic data",
...
tibble(provider = c("Integrated Taxonomic Information System (ITIS 2019)",
  ...
  knitr::kable(format = "latex", col.names = c("Provider", "Abbreviation", "Number of \nIdentifiers", "Description"), escape = FALSE, 
               caption = "Descriptions of providers supported by taxaDB.") %>%
...
Some taxonomic name providers (e.g. OTT, COL) offer periodic releases of a static names list, while many other providers (e.g. ITIS, NCBI, FB, IUCN) offer name data on a rolling basis (i.e. the data returned by a given download URL is updated continuously or at arbitrary intervals without any additional indication if and how that data has changed.)  `taxadb`'s `td_create()` function downloads and stores cached snapshopts from each provider, which follow an annual release model to support reproducible analyses.  By default, `taxadb` will determine the latest version available (at the time of writing this is version `2019`), and install and use this version of all snapshots for each provider.  However, all taxadb functions can take   
