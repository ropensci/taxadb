...
potential ways for resolving unidentified names
* multiple hits from `get_ids()` check duplicates, use `by_`
* typo
* formatting issues (be careful about false matches), `clean_names()`
* ambiguous resolution, explore with `synonyms()` (these are the same concept with different names or vice versa)

The spirit of taxadb is not to automate steps that require expert knowledge, but provide access to multiple potential "taxonomic theories"
