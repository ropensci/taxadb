...
Notice that one species name could not be resolved to a COL identifier.  This could indicate a data entry problem such as a misspelling of a name or the inclusion of a citation or year, which may be addressed by `clean_names()`. However, failure to resolve properly specified scientific names is not uncommon, as even large scale efforts like the Catalogue of Life are far from a comprehensive list of all published scientific names.  
...
A name unresolved by 'get_ids' could also be the result of a name matching to multiple accepted names, as in the case of a species that has been split. By design, these cases are left to be resolved by the researcher using the 'by_' functions to filter underlying taxonomic tables for additional information. 

```{r}
#by_name() w/multiple matches example.
```
'by_' functions can also be used to filter by common name, rank, etc...

```{r}
by_rank("Aves", rank="class", provider = "col") %>%
  ## some example columns
  select(taxonID, taxonomicStatus, taxonRank, genus, specificEpithet) %>% 
  head(5)

```


We can inspect all columns: 

```{r}
by_rank("Aves", rank="class", provider = "col") %>% names()
```

Note that each of these terms are defined by 
...
Some synonyms may also be associated with more than one unique identifier.  This situation is more difficult, as it is impossible to know which accepted name should be used for such synonyms without more context.  The `get_ids()` function will thus return `NA` for any name that matches more than one identifier, just as it will return `NA` for a name that does not match any identifier at all.  


Other functions in `taxadb` allow us to explore these names further, for example having access to a local database makes it easy to explore such issues as "identify all synonyms that resolve to more than one accepted scientific name ID", like so: 
