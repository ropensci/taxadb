...
To illustrate this, we consider the following ten species names of different birds:

<!-- Would this example be more compelling if we were joining two tables of data?  -->
...
The `get_ids()` function in `taxadb` can quickly resolve thousands of species names to unique identifiers. Here, we see that resolving names to identifiers in ITIS reveals that we have only 5 unique ids, as many of our names were known synonyms:
...
```{r}
birds <- birds %>% mutate(id = get_ids(species, "itis")) %>% arrange(id)
birds
```
...
Here we see that  `"Myadestes elizabeth"` was likely a miss-spelling of the ITIS-accepted name `"Myadestes elisabeth"`.  

```{r}
get_ids("Myadestes elisabeth", provider = "itis", format = "uri")
```
 


It is equally important to be able to resolve scientific names from these identifiers.   
...
We can get a richer picture of the our matched names by using the table-functions such as `by_id` to return a table of taxonomic information about each identifier.  For instance, with this information we can quickly verify that no name has been mistakenly matched to a taxon outside of the target group (*Aves*): 
 
 <!-- show multiple-matched cases -->
 
```{r}
df <- by_id(birds$id, "itis") 
df 
```

The column headings (though not necessarily the order) are consistent across `taxadb`, with all column names using terms defined by the Darwin Core ontology, [@dwc].  Some of these (`acceptedNameUsageID`, `taxonRank`, `taxonomicStatus`) may be cumbersome, but are at least consistent and unambiguous.  `taxadb` promises a core set of these columns are provided for (see Table 3)


```{r}
df %>% filter(class != "Aves")
```
...
```{r}
by_name(birds$species)
```


```{r}
by_rank("Aves", rank="class")
```
