...
`get_ids` by default looks only for exact matches.  We can get more matches by cleaning the names first.  `clean_names()` 

```{r}
clean_names(bbs$species, 
            )
```

```{r}
ids <- get_ids(clean_names(bbs$species), "ott", "prefix")
unmatched <- bbs$species[is.na(ids)]
clean_names(unmatched)
```

```{r}
unmatched <- bbs$species[is.na(bbs_ids$ott)]
length(unmatched)
...
bbs_ids %>% 
  select(-species) %>% 
  purrr::transpose() %>%
  purrr::map_dbl(function(x) sum(!is.na(x)) < 1) %>% sum()
```
